...
public class RandomBoard {
    ...
    public int m; //列数
    public int o;// 零的个数
    public int b;//block结块的个数
    public int runtime = 0; //假定的最大运行次数
    public Random random = new Random();//生成随机数
    ...
    public RandomBoard(int n, int m, int b) {
        ...
        由于block结块最大为2*2，故设置零的个数上限为2下限为1
         */
        initial();//初始化棋盘
    }
    ...
    public void initial() {
        this.o = random.nextInt(1, 3);//随机给定零的个数
        int k = 0;
        int[] num = new int[n * m];
       ...
       初始化棋盘，设定node中棋盘序列num[]和零的位置zero[]
         */
        ArrayList<Block> blocks = new ArrayList<>();
        int z = 1;
        ArrayList<Integer> list = new ArrayList<>();
        //设置一个链表，存储已经结块的数字，判断下一个结块与上一个结块有没有区域相连，以排除非法结块
        while (z <= b) {//
...
//下面将根据结块大小设置一个随机的合理位置（随机设置结块的首位）
            if (a1 == 1 && a2 == 2) {
                while (true) {
                    element = new int[2];
